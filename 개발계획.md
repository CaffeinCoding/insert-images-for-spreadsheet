...
### 6.2 Progress 표시 및 완료 버튼 상태

#### 세부 과제
- [ ] Progress 바 UI 생성
- [ ] 진행률 계산 함수
- [ ] Progress 색상 변경 (정상/에러)
- [ ] 완료 버튼 상태 관리
  - [ ] 비활성화 조건 확인
  - [ ] 완료 → 저장 상태 전환
- [ ] 취소 버튼 표시/숨김
...
// 함수 목록
- handleImageInsertionError(error, imageIndex)
- displayErrorMessage(errorCount, totalImages)
- handleErrorRecovery()
- restorePreviewAfterError()
```

#### 에러 메시지 포맷
```
"실패한 이미지 1/10: image_name.jpg"
```
...
- undoImageInsertion()
- restoreUIState()
- SpreadsheetApp.undo()  // Google Apps Script API
```
...
| 4 | 이미지 크기 + 설정 | 3~4일 | 🔴 높음 | ⏳ 예정 |
| 5 | localStorage + 고급 기능 | 3~4일 | 🟠 중간 | ⏳ 예정 |
| 6 | 이미지 삽입 + 에러 처리 | 2~3일 | 🔴 높음 | ⏳ 예정 |
